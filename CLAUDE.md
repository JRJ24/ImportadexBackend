...
IMPORTADEX_ADMIN_EMAILS     # direcciones extra (coma-separadas) sumadas a las notificaciones internas, fuera de la tabla de MIREX
IMPORTADEX_NOTIFY_EMAILS    # idem, agregado por el trabajo de client-portal-login (45e3d75/418959b), no de esta sesion
...
- **Ticket con soporte de Bluehost (`E-570847`) — CERRADO (2026-08-04).** Su equipo escalado identificó correctamente la causa (HELO loopback) revisando sus logs de Exim y sugirió el mismo fix que se aplicó. Se les respondió confirmando la resolución (texto final en `ticket-bluehost-cierre.txt`) solicitando el cierre formal del ticket.
...
**Destinatarios internos reales (9, no 7):** `getImportadexNotificationUsers()` (MIREX DB) devuelve 7 (3 `ADMIN` + 4 `OPERACIONES`), pero `sendImportadexInternalNotification()` (agregado por el trabajo de `45e3d75`/`418959b`, ajeno a esta sesión) les suma además `IMPORTADEX_ADMIN_EMAILS` y `IMPORTADEX_NOTIFY_EMAILS` (env vars) — en este droplet, `jonaifry.rodriguez@flypack.com.do` y `rjstudios24012006@gmail.com` respectivamente. Total real: 9 intentos por notificación interna (1 se filtra por dominio local).

**Fix iteración 1 (insuficiente):** `mapWithConcurrency()` limitando a 2 conexiones simultáneas. Se volvió a probar en producción y **fallaron los mismos 3 destinatarios** (siempre las últimas 3 posiciones del arreglo: `laura.ciriaco`, `rjstudios24012006@gmail.com`, `jonaifry.rodriguez@flypack.com.do`).

**Fix iteración 2 (insuficiente):** concurrencia bajada a 1 (totalmente serial) + reintentos automáticos (`SMTP_CONNECTION_RETRIES`) en errores de conexión transitorios. Se probó de nuevo: **fallaron exactamente los mismos 3, en las mismas posiciones**, sin importar concurrencia ni reintentos. Esto descarta definitivamente que sea un límite de conexiones *simultáneas* — un `ECONNREFUSED` en el connect ocurre antes de que el servidor sepa quién es el destinatario, así que no puede depender de eso. Es un límite de **cuántas conexiones nuevas** tolera Bluehost en una ráfaga corta (rate limit), no de concurrencia — por eso siempre fallan las últimas posiciones de una lista de 8-9 envíos consecutivos, sin importar si son en paralelo o en serie.

**Fix aplicado (tercera iteración, la correcta, 2026-08-04):** se reescribió la rama `privateRecipients` de `sendMail()` para enviar **un solo correo con todos los destinatarios en BCC** (una sola conexión SMTP) en vez de una conexión por destinatario. Preserva la privacidad original (nadie ve el email de los demás, sigue siendo BCC) pero elimina el problema de raíz: ya no importa cuántos destinatarios haya, siempre es 1 conexión. Se eliminó `mapWithConcurrency()`/`SMTP_MAX_CONCURRENT_SENDS` (quedaron obsoletos, ya no hay nada que throttlear). Se mantiene `SMTP_CONNECTION_RETRIES` (reintento en errores de conexión transitorios), útil para cualquier envío, no solo el de destinatarios múltiples.

**Pendiente:** confirmar en producción con una operación real que los 9 destinatarios (incluyendo `laura.ciriaco`) reciban el correo de forma consistente con esta tercera iteración.
