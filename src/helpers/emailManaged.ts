...
const sendTransportMail = (mailOptions: Record<string, unknown>) =>
  (transporter.sendMail as unknown as (options: Record<string, unknown>) => Promise<SentMessageInfo>)(mailOptions);
...
const getConfiguredInternalRecipients = () =>
  uniqueEmails([
  ...
  try {
    ...
    if (privateRecipients) {
      ...
      const rejected = results.reduce((total, result) => {
        if (result.status !== "fulfilled") return total + 1;
