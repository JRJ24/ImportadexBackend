...
async function seedOperation() {
    ...
    VALUES ($1, $2, $3, 'IMPORT', 'SEA', 'CONTAINERIZED', 'IN_CUSTOMS', $4, $5, $6, $7, $8, $9, $10, $11, $12, now())`, id, "IMP-2407-018", "Repuestos Andinos", "Documentos en revision", "Alta", "Ningbo, CN", "La Paz, BO", "Arica", "MSC", "BL MSCU9328741", new Date("2026-06-14T00:00:00.000Z"), 62);
    return id;
}
async function seedChildren(operationId) {
    ...
    if ((countRows[0]?.total ?? 0) > 0)
        return;
    await connectionDB_1.prisma.$executeRawUnsafe(`INSERT INTO importadex_containers (id, operation_id, number, type, seal, carrier, free_days, return_limit, status, updated_at)
     VALUES ($1, $2, $3, $4, $5, $6, 7, $7, $8, now())`, (0, crypto_1.randomUUID)(), operationId, "MSCU8734210", "40HC", "SL-90412", "MSC", new Date("2026-06-23T00:00:00.000Z"), "Pendiente de devolucion");
    await connectionDB_1.prisma.$executeRawUnsafe(`INSERT INTO importadex_cargo_items (id, operation_id, package_type, pieces, pallets, weight_kg, volume_cbm, handling, updated_at)
     VALUES ($1, $2, $3, 84, 18, 21480, 58, $4, now())`, (0, crypto_1.randomUUID)(), operationId, "Carga paletizada", "Montacargas");
    for (const [name, type, status, owner] of [
        ...
        await connectionDB_1.prisma.$executeRawUnsafe(`INSERT INTO importadex_documents (id, operation_id, name, type, status, owner, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, now())`, (0, crypto_1.randomUUID)(), operationId, name, type, status, owner);
    }
    await connectionDB_1.prisma.$executeRawUnsafe(`INSERT INTO importadex_customs_files (id, operation_id, declaration_no, regime, channel, status, responsible, updated_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7, now())`, (0, crypto_1.randomUUID)(), operationId, "DIM-2026-9182", "Importacion consumo", "Rojo", "PENDING_REVIEW", "Aduanas");
    await connectionDB_1.prisma.$executeRawUnsafe(`INSERT INTO importadex_incidents (id, operation_id, type, severity, status, owner, description, updated_at)
     VALUES ($1, $2, $3, 'MEDIUM', 'OPEN', $4, $5, now())`, (0, crypto_1.randomUUID)(), operationId, "Pago pendiente", "Aduanas", "Esperando confirmacion de pago del cliente.");
