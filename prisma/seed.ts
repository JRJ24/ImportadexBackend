...
import { prisma } from "../src/config/connectionDB";
  ...
  for (const [group, rows] of Object.entries(catalogs)) {
    for (const [value, label] of rows) {
      await prisma.$executeRawUnsafe(
        `INSERT INTO importadex_catalogs (id, "group", value, label)
         VALUES ($1, $2, $3, $4)
...
async function seedOperation() {
  ...
  const existing = await prisma.$queryRawUnsafe<{ id: string }[]>(
    "SELECT id FROM importadex_operations WHERE code = $1",
    "IMP-2407-018",
  );
...
async function seedChildren(operationId: string) {
  const countRows = await prisma.$queryRawUnsafe<{ total: number }[]>(
    "SELECT COUNT(*)::int AS total FROM importadex_events WHERE operation_id = $1",
    operationId,
  );
  ...
  await prisma.$executeRawUnsafe(
    ...
    "Pago pendiente",
    "Aduanas",
    "Esperando confirmacion de pago del cliente.",
  );
  ...
  for (const [event, owner, location, date] of [
    ...
    await prisma.$executeRawUnsafe(
      `INSERT INTO importadex_events (id, operation_id, event, owner, location, event_date)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      randomUUID(),
      operationId,
      event,
